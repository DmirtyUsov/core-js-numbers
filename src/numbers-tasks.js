...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(dot12 / (magnitude1 * magnitude2));
}
...
function isPrime(n) {
  ...
  if (n < 2) {
    ...
    // even numbers. As all primes except 2 are odd
    result = false;
  } else if (n < 9) {
    // 5, 7
    result = true;
    ...
    // 5 and 7, 11 and 13, 17 and 19  => 5 and 5+2, 5+6 and 5+6+2
    // start with p = 5
    // n = a * b;  a and b can't be both greater than the square root of n
    // n = sqrt(n) * sqrt(n);
    while (p <= Math.sqrt(n)) {
      if (n % p === 0) {
        result = false;
...
function getSumToN(n) {
  return Array.from({ length: n + 1 }, (value, index) => index).reduce(
    (a, b) => a + b,
    0
  );
}
